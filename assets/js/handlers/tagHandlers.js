import { createTag, updateTag, deleteTag, associateTagWithCard, getTagsFromAPI } from '../tags/api.tags.module.js';
...
async function handleAssociateTagForm() {
    const associateTagForm = document.getElementById('associateTagForm');
    associateTagForm.addEventListener('submit', async (event) => {
        event.preventDefault();
        const data = new FormData(associateTagForm);
        const cardId = data.get('cardId');
        const tagId = data.get('tagId');
        await associateTagWithCard(cardId, tagId);
        window.location.reload(); // Refresh to reflect changes
    });
}

function populateTagsSelect(tags) {
    const tagSelectElements = document.querySelectorAll('select[name="selectedTag"]');
...
export { showAddTagModal, showEditTagModal, showAssociateTagModal, hideModals, handleAddTagForm, handleEditTagForm, handleAssociateTagForm, loadAndPopulateTags};
