import { createList, updateList, deleteList } from '../lists/api.lists.module.js';
...
async function handleAddListForm() {
    ...
    addListForm.addEventListener('submit', async (event) => {
        ...
        window.location.reload(); // Refresh to load new list
    });
}
...
async function handleEditListForm() {
    ...
    editListForms.forEach(form => {
        form.addEventListener('submit', async (event) => {
            ...
            window.location.reload(); // Refresh to load updated list
        });
    });
}
...
export { showAddListModal, showEditListForm, handleAddListForm, handleEditListForm, handleDeleteList };
